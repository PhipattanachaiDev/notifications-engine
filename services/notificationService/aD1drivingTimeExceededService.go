...
type DrivingTimeExceededAD1APIResponse struct {
	Success   bool                  `json:"success"`
	Status    int                   `json:"status"`
	Timestamp string                `json:"timestamp"`
	Message   string                `json:"message"`
	Data      []DrivingTimeExceededAD1 `json:"data"`
}
type DrivingTimeExceededAD1 struct {
...
func DrivingTimeExceededAD1Notifications(startTime time.Time, settings map[string]interface{}, userNotifySetting *[]userService.UserNotifySetting) {
	...
	for i, user := range *userNotifySetting {
		...
		// Fetch drivingTimeExceededAD1 notifications for the user
		drivingTimeExceededAD1Notifications, err := getDrivingTimeExceededAD1Notifications(user.UserID, lastDateTime.Format(time.RFC3339))
		if err != nil {
			logService.WriteLog("driving_time_exceeded_ad1_ad1_error", fmt.Sprintf("failed to fetch driving Time Exceeded notifications for user %d: %v", user.UserID, err))
			continue
		}

		// ตรวจสอบว่ามีการแจ้งเตือนใน drivingTimeExceededAD1Notifications หรือไม่
		if len(drivingTimeExceededAD1Notifications) > 0 {
			// อัพเดท LastMessageGPSDateTime ด้วยเวลาของการแจ้งเตือนล่าสุด
			latestNotification := drivingTimeExceededAD1Notifications[len(drivingTimeExceededAD1Notifications)-1]
			...
			if err != nil {
				logService.WriteLog("driving_time_exceeded_ad1_ad1_error", fmt.Sprintf("failed to parse time: %v", err))
				continue
			}
		...
		messages := formatDrivingTimeExceededAD1Notifications(drivingTimeExceededAD1Notifications)
	...
	logService.WriteLog("driving_time_exceeded_ad1", finalLogMessage)
}

func getDrivingTimeExceededAD1Notifications(userId int, lastDateTime string) ([]DrivingTimeExceededAD1, error) {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	requestBody := map[string]interface{}{
		...
		"userId":               userId,
		"messageType":          []string{"AD1"},
		"eventId":              6025,
		"deleteLogMessageType": []string{"AD1"},
	}
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err != nil {
			if i == maxRetries-1 {
				return nil, fmt.Errorf("error sending to get Driving Time Exceeded AD1 Notifications after %d retries: %w", maxRetries, err)	
			}
		...
		var apiResponse DrivingTimeExceededAD1APIResponse
...
func formatDrivingTimeExceededAD1Notifications(notifications []DrivingTimeExceededAD1) []string {
	var messages []string
	...
	for _, notification := range notifications {
		...
		// if notification.
		message := fmt.Sprintf(
			"แจ้งเตือนขับต่อเนื่องสะสมเกิน 4 ชั่วโมง\n"+
				"เลขทะเบียน: %s\n"+
				"วันเวลา: %s\n"+
				"ความเร็ว %d\n"+
				"สถานที่: %s\n"+
				"https://www.google.com/maps?q=%s,%s", notification.LicenseNo, notification.GPSDateTime, notification.Speed, notification.LocationTH, notification.Latitude, notification.Longitude)
