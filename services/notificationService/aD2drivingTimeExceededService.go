...
type DrivingTimeExceededAD2APIResponse struct {
	Success   bool                     `json:"success"`
	Status    int                      `json:"status"`
	Timestamp string                   `json:"timestamp"`
	Message   string                   `json:"message"`
	Data      []DrivingTimeExceededAD2 `json:"data"`
}
type DrivingTimeExceededAD2 struct {
...
func DrivingTimeExceededAD2Notifications(startTime time.Time, settings map[string]interface{}, userNotifySetting *[]userService.UserNotifySetting) {
	...
	for i, user := range *userNotifySetting {
		...
		// Fetch drivingTimeExceededAD2 notifications for the user
		drivingTimeExceededAD2Notifications, err := getDrivingTimeExceededAD2Notifications(user.UserID, lastDateTime.Format(time.RFC3339))
		if err != nil {
			logService.WriteLog("driving_time_exceeded_ad2_error", fmt.Sprintf("failed to fetch driving Time Exceeded notifications for user %d: %v", user.UserID, err))
			continue
		}

		// ตรวจสอบว่ามีการแจ้งเตือนใน drivingTimeExceededAD2Notifications หรือไม่
		if len(drivingTimeExceededAD2Notifications) > 0 {
			// อัพเดท LastMessageGPSDateTime ด้วยเวลาของการแจ้งเตือนล่าสุด
			latestNotification := drivingTimeExceededAD2Notifications[len(drivingTimeExceededAD2Notifications)-1]
			...
			if err != nil {
				logService.WriteLog("driving_time_exceeded_ad2_error", fmt.Sprintf("failed to parse time: %v", err))
				continue
			}
		...
		messages := formatDrivingTimeExceededAD2Notifications(drivingTimeExceededAD2Notifications)
	...
	logService.WriteLog("driving_time_exceeded_ad2", finalLogMessage)
}

func getDrivingTimeExceededAD2Notifications(userId int, lastDateTime string) ([]DrivingTimeExceededAD2, error) {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	requestBody := map[string]interface{}{
		...
		"userId":               userId,
		"messageType":          []string{"AD2"},
		"eventId":              6025,
		"deleteLogMessageType": []string{"AD2"},
	}
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err != nil {
			if i == maxRetries-1 {
				return nil, fmt.Errorf("error sending to get Driving Time Exceeded AD2 Notifications after %d retries: %w", maxRetries, err)	
			}
		...
		var apiResponse DrivingTimeExceededAD2APIResponse
...
func formatDrivingTimeExceededAD2Notifications(notifications []DrivingTimeExceededAD2) []string {
	var messages []string
	...
	for _, notification := range notifications {
		...
		// if notification.
		message := fmt.Sprintf(
			"แจ้งเตือนขับต่อเนื่องสะสมเกิน 10 ชั่วโมง\n"+
				"เลขทะเบียน: %s\n"+
				"วันเวลา: %s\n"+
				"ความเร็ว %d\n"+
				"สถานที่: %s\n"+
