...
type ParkOffOutsideSpotAPIResponse struct {
	Success   bool                 `json:"success"`
	Status    int                  `json:"status"`
	Timestamp string               `json:"timestamp"`
	Message   string               `json:"message"`
	Data      []ParkOffOutsideSpot `json:"data"`
}

type ParkOffOutsideSpot struct {
...
func ParkAndTurnOffEngineOutsideDesignatedParkingArea(startTime time.Time, settings map[string]interface{}, userNotifySetting *[]userService.UserNotifySetting) {
	...
	for i, user := range *userNotifySetting {
		...
		ParkAndTurnOffEngineOutsideParking, err := getParkAndTurnOffEngineOutsideDesignatedParkingAreaNotificationsByUserId(user.UserID)
		if err != nil {
			logService.WriteLog("park_and_turn_off_outside_parking_error", fmt.Sprintf("failed to get park and stop outside spot notifications for user %d: %v", user.UserID, err))
			continue
		}
		
		// ตรวจสอบว่ามีการแจ้งเตือนใน parkAndTurnOffEngineOutsideDesignatedParkingAreaNotifications หรือไม่
		if len(ParkAndTurnOffEngineOutsideParking) > 0 {
			// อัพเดท LastMessageGpsDateTime ด้วยเวลาของการแจ้งเตือนล่าสุด
			latestNotification := ParkAndTurnOffEngineOutsideParking[len(ParkAndTurnOffEngineOutsideParking)-1]
			// Correct time format for parsing the input
			layout := "02/01/2006 15:04:05"

			parsedTime, err := time.Parse(layout, latestNotification.GPSDateTime)
			if err != nil {
				logService.WriteLog("park_and_turn_off_outside_parking_error", fmt.Sprintf("failed to parse time: %v", err))
				continue
			}
			(*userNotifySetting)[i].LastMessageGPSDateTime = parsedTime
		}

		messages := formatParkAndTurnOffEngineOutsideDesignatedParkingAreaNotification(ParkAndTurnOffEngineOutsideParking)
		...
		err = parkAndTurnOffEngineOutsideDesignatedParkingAreaLogs(user.UserID, ParkAndTurnOffEngineOutsideParking)
		if err != nil {
			logService.WriteLog("park_and_turn_off_outside_parking_error", fmt.Sprintf("failed to insert notifications log for user %d: %v", user.UserID, err))
			continue
		}
	}
	...
	logService.WriteLog("park_and_turn_off_outside_parking", finalLogMessage)
}

func formatParkAndTurnOffEngineOutsideDesignatedParkingAreaNotification(notifications []ParkOffOutsideSpot) []string {
	var messages []string
	...
	for _, notification := range notifications {
		message := fmt.Sprintf(
			"แจ้งเตือนจอดดับเครื่องนอกจุดจอดในสถานที่กำหนด (POI)\n"+
				"เลขทะเบียน: %s\n"+
				"วันเวลาดับเครื่อง: %s\n"+
				"วันเวลาปัจจุบัน: %s\n"+
				"สถานที่: %s\n"+
...
func getParkAndTurnOffEngineOutsideDesignatedParkingAreaNotificationsByUserId(userId int) ([]ParkOffOutsideSpot, error) {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err != nil {
			if i == maxRetries-1 {
				return nil, fmt.Errorf("error sending to Park And Turn Off Engine Outside Designated Parking Area Notifications after %d retries: %w", maxRetries, err)
			}
		...
		var apiResponse ParkOffOutsideSpotAPIResponse
...
func parkAndTurnOffEngineOutsideDesignatedParkingAreaLogs(userId int, ParkOffOutsideSpot []ParkOffOutsideSpot) error {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	for _, parkOffOutsideSpots := range ParkOffOutsideSpot {

		t, _ := time.Parse("02/01/2006 15:04:05", parkOffOutsideSpots.GPSDateTime)
		...
		requestBody := map[string]interface{}{
			"userId":               userId,
			"vehicleId":            parkOffOutsideSpots.VehicleID,
			"eventId":              6006,
			"gpsDate":              gpsDate,
			"gpsTime":              gpsTime,
			"latitude":             parkOffOutsideSpots.Latitude,
			"longitude":            parkOffOutsideSpots.Longitude,
			"locationTH":           parkOffOutsideSpots.LocationTH,
			"waypointLocationId":   0,
			"waypointLocationName": parkOffOutsideSpots.WaypointLocationNameTH,
