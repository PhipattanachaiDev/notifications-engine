...
type PkWithEngineRunningOutSideAPIResponse struct {
	Success   bool                                     `json:"success"`
	Status    int                                      `json:"status"`
	Timestamp string                                   `json:"timestamp"`
	Message   string                                   `json:"message"`
	Data      []PkWithEngineRunningExtendedOutsidePark `json:"data"`
}
type PkWithEngineRunningExtendedOutsidePark struct {
...
func ParkedWithEngineRunningExtendedOutsideParking(startTime time.Time, settings map[string]interface{}, userNotifySetting *[]userService.UserNotifySetting) {
	...
	for i, user := range *userNotifySetting {
		...
		// }
		PkWithEngineRunningExtendedOutsideParking, err := getParkedWithEngineRunningExtendedOutsideParkingNotificationsByUserId(user.UserID)
		if err != nil {
			logService.WriteLog("parked_with_engine_running_extended_outside_parking_error", fmt.Sprintf("failed to get parked with engine running extended outside parkingnotifications for user %d: %v", user.UserID, err))
			continue
		}
		
		// ตรวจสอบว่ามีการแจ้งเตือนใน PkWithEngineRunningExtendedOutsideParking หรือไม่
		if len(PkWithEngineRunningExtendedOutsideParking) > 0 {
			// อัพเดท LastMessageGpsDateTime ด้วยเวลาของการแจ้งเตือนล่าสุด
			latestNotification := PkWithEngineRunningExtendedOutsideParking[len(PkWithEngineRunningExtendedOutsideParking)-1]
			// Correct time format for parsing the input
			layout := "02/01/2006 15:04:05"

			parsedTime, err := time.Parse(layout, latestNotification.GPSDateTime)
			if err != nil {
				logService.WriteLog("parked_with_engine_running_extended_outside_parking_error", fmt.Sprintf("failed to parse time: %v", err))
				continue
			}
			(*userNotifySetting)[i].LastMessageGPSDateTime = parsedTime
		}

		messages := formatParkParkedWithEngineRunningExtendedOutsideParkingNotifications(PkWithEngineRunningExtendedOutsideParking)
		...
		err = PkWithEngineRunningExtendedOutsideParkingLogs(user.UserID, PkWithEngineRunningExtendedOutsideParking)
		if err != nil {
			logService.WriteLog("parked_with_engine_running_extended_outside_parking_error", fmt.Sprintf("failed to insert notifications log for user %d: %v", user.UserID, err))
			continue
		}
	}
	...
	logService.WriteLog("parked_with_engine_running_extended_outside_parking", finalLogMessage)
}

func getParkedWithEngineRunningExtendedOutsideParkingNotificationsByUserId(userId int) ([]PkWithEngineRunningExtendedOutsidePark, error) {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err != nil {
			if i == maxRetries-1 {
				return nil, fmt.Errorf("error sending to Park With Engine Running Period Outside Area Notifications after %d retries: %w", maxRetries, err)
			}
		...
		var apiResponse PkWithEngineRunningOutSideAPIResponse
...
func formatParkParkedWithEngineRunningExtendedOutsideParkingNotifications(notifications []PkWithEngineRunningExtendedOutsidePark) []string {
	var messages []string
	...
	for _, notification := range notifications {
		message := fmt.Sprintf(
			"แจ้งเตือนจอดติดเครื่องนานเกินกำหนดนอกจุดจอด\n"+
				"เลขทะเบียน: %s\n"+
				"วันเวลาดับเครื่อง: %s\n"+
				"วันเวลาปัจจุบัน: %s\n"+
				"สถานที่: %s\n"+
				"https://www.google.com/maps?q=%s,%s", notification.LicenseNo, notification.EngineOffDateTime, utcPlus7, notification.LocationTH, notification.Latitude, notification.Longitude)
		messages = append(messages, message)
	}
	return messages
}

func PkWithEngineRunningExtendedOutsideParkingLogs(userId int, PkEngineRunningOutside []PkWithEngineRunningExtendedOutsidePark) error {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	for _, pk := range PkEngineRunningOutside {

		t, _ := time.Parse("02/01/2006 15:04:05", pk.GPSDateTime)
		...
		requestBody := map[string]interface{}{
			"userId":               userId,
			"vehicleId":            pk.VehicleID,
			"eventId":              6005,
			"gpsDate":              gpsDate,
			"gpsTime":              gpsTime,
			"latitude":             pk.Latitude,
			"longitude":            pk.Longitude,
			"locationTH":           pk.LocationTH,
			"waypointLocationId":   0,
			"waypointLocationName": pk.WaypointLocationNameTH,
