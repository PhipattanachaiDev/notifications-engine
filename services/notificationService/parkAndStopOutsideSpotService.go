...
type PkAPIResponse struct {
	...
	Data      []ParkingNotifications `json:"data"`
}

type ParkingNotifications struct {
...
func ParkAndStopOutsideSpot(startTime time.Time, settings map[string]interface{}, userNotifySetting *[]userService.UserNotifySetting) {
	...
	for i, user := range *userNotifySetting {
		...
		parkingNotifications, err := getParkAndStopOutSideSpotNotificationsByUserId(user.UserID)
		if err != nil {
			logService.WriteLog("park_and_stop_outside_spot_error", fmt.Sprintf("Failed to fetch park and stop outside spot notifications for user %d: %v", user.UserID, err))
			continue
		}

		// ตรวจสอบว่ามีการแจ้งเตือนใน parkingNotifications หรือไม่
		if len(parkingNotifications) > 0 {
			// อัปเดต `LastMessageGPSDateTime` ด้วยเวลาของการแจ้งเตือนล่าสุด
			latestNotification := parkingNotifications[len(parkingNotifications)-1]
			...
			if err != nil {
				logService.WriteLog("park_and_stop_outside_spot_error", fmt.Sprintf("failed to parse time: %v", err))
				continue
			}
			(*userNotifySetting)[i].LastMessageGPSDateTime = parsedTime
		}

		isSendTelegram := user.IsSendTelegram

		// จัดรูปแบบข้อความการแจ้งเตือน
		messages := pkformatNotifications(parkingNotifications)
		...
		err = pkNotificationsLogs(user.UserID, parkingNotifications)
		if err != nil {
			logService.WriteLog("park_and_stop_outside_spot_error", fmt.Sprintf("failed to insert notifications log for user %d: %v", user.UserID, err))
			continue
		}
	}

	// บันทึกข้อมูลจำนวนข้อความที่ส่งได้และข้อผิดพลาด log
	finalLogMessage := fmt.Sprintf("Total notifications sent: %d, Total errors: %d", totalNotificationsSent, totalErrors)
	logService.WriteLog("park_and_stop_outside_spot", finalLogMessage)
}

func getParkAndStopOutSideSpotNotificationsByUserId(userId int) ([]ParkingNotifications, error) {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err != nil {
			if i == maxRetries-1 {
				return nil, fmt.Errorf("error sending to Park And Stop Outside Spot Notifications after %d retries: %w", maxRetries, err)
			}
...
func pkformatNotifications(notifications []ParkingNotifications) []string {
	var messages []string
	...
	for _, notification := range notifications {
		message := fmt.Sprintf(
			"แจ้งเตือนจอดดับเครื่องนอกจุดจอดเกิน 5 นาที\n"+
				"เลขทะเบียน:  %s\n"+
				"วันเวลาดับเครื่อง: %s\n"+
				"วันเวลาปัจจุบัน: %s\n"+
				"สถานที่: %s\n"+
...
func pkNotificationsLogs(userId int, parking []ParkingNotifications) error {
	// Load .env file
	err := godotenv.Load()
	if err != nil {
	...
	for _, parkings := range parking {

		waypointLocationName := parkings.WaypointLocationNameTH

		t, _ := time.Parse("02/01/2006 15:04:05", parkings.GPSDateTime)
		...
		requestBody := map[string]interface{}{
			"userId":               userId,
			"vehicleId":            parkings.VehicleID,
			"eventId":              6004,
			"gpsDate":              gpsDate,
			"gpsTime":              gpsTime,
			"latitude":             parkings.Latitude,
			"longitude":            parkings.Longitude,
			"locationTH":           parkings.LocationTH,
			"waypointLocationId":   0,
			"waypointLocationName": waypointLocationName,
		...
		req, err := http.NewRequest("POST", fmt.Sprintf("%s/notifications/vehicle-notification/log", apiURL), bytes.NewReader(bodyBytes))
		if err != nil {
